...
describe("AuthService WebSocket challenge", () => {
  ...
  it("rejects signature from wrong keypair", () => {
    ...
    expect(() => service.verifyWsChallenge(address, challenge, timestamp, signature)).toThrow(
      AuthError
    );
  });
  ...
  it("rejects tampered challenge", () => {
    const { challenge, timestamp } = service.createWsChallenge(address);
    const signature = signChallenge(challenge, kp);
    expect(() => service.verifyWsChallenge(address, challenge + "X", timestamp, signature)).toThrow(
      AuthError
    );
  });
  ...
  it("rejects expired timestamp", () => {
    ...
    expect(() => service.verifyWsChallenge(address, challenge, oldTimestamp, signature)).toThrow(
      AuthError
    );
  });
  ...
  it("rejects invalid challenge format", () => {
    ...
    expect(() => service.verifyWsChallenge(address, fakeChallenge, timestamp, signature)).toThrow(
      AuthError
    );
  });
  ...
  it("rejects short signature", () => {
    const { challenge, timestamp } = service.createWsChallenge(address);
    expect(() => service.verifyWsChallenge(address, challenge, timestamp, "deadbeef")).toThrow(
      AuthError
    );
  });
});
