...
function generateClient(
  ...
  if (readMethods.length > 0) {
    ...
    for (const fn of readMethods) {
      ...
      // Traceability docstring (Epta-Node#1365): every generated method
      // references its originating contract function.
      lines.push("  /**");
      lines.push(`   * Contract entrypoint: \`@${fn.name}\`.`);
      lines.push(`   * SDK method: \`${camelCase(fn.name)}\`.`);
      lines.push("   */");
      lines.push(
  ...
  if (writeMethods.length > 0) {
    ...
    for (const fn of writeMethods) {
      const pascalName = snakeToPascal(fn.name);
      // Traceability docstring (Epta-Node#1365).
      lines.push("  /**");
      lines.push(`   * Contract entrypoint: \`@${fn.name}\`.`);
      lines.push(`   * SDK method: \`${camelCase(fn.name)}\`.`);
      lines.push("   */");
...
/**
 * Parity map generation (Epta-Node#1365): every generated SDK method is
 * recorded with the contract entrypoint it originates from, and every
 * generated event interface with the contract event it originates from.
 * `contract-parity.test.ts` asserts the map is complete and that every
 * mapped entrypoint/event still exists in the contract source, flagging any
 * unmapped SDK surface.
 */
function generateParity(
  ...
  for (const fn of functions) {
    lines.push(`  ${camelCase(fn.name)}: "${fn.name}",`);
  }
  lines.push("} as const;");
  lines.push("");
  lines.push("/**");
  lines.push(" * Map of generated event interface name -> originating contract event");
  lines.push(" * struct name in lib.rs.");
  lines.push(" */");
  lines.push("export const CONTRACT_EVENT_MAP: Readonly<Record<string, string>> = {");
  for (const ev of events) {
    lines.push(`  ${ev.name}: "${ev.name}",`);
  }
  lines.push("} as const;");
  lines.push("");
  return lines.join("\n");
...
function main() {
  ...
  // Also generate the barrier file. It must stay side-effect-free so that
  // bundlers can tree-shake generated exports (Epta-Node#1364).
  writeFileSync(
  ...
  // Parity map used by the SDK parity test (Epta-Node#1365).
  const parityContent = generateParity(functions, events);
  writeFileSync(resolve(OUT_DIR, "contract-parity.ts"), parityContent);
  console.log("  ✓ packages/sdk/src/generated/contract-parity.ts");
